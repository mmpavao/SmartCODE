...
export function useDataOperations({
  ...
  const handleExportSelectedSettings = useCallback(
    async (categoryIds: string[]) => {
      ...
      try {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `koda-settings-${categoryIds.join('-')}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
  ...
  const handleExportSelectedChats = useCallback(
    async (chatIds: string[]) => {
      ...
      try {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'koda-selected-chats.json';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
  ...
  const handleDownloadTemplate = useCallback(async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = 'koda-api-keys-template.json';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
