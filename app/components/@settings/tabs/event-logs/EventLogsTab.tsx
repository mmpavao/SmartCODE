...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `koda-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `koda-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with Koda
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('Koda - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by Koda', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`koda-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `koda-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
